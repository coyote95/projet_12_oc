...
class EventView(BaseView):
    ...
    def input_start_date():
        while True:
            start_date = input(
                "Entrez la date de début de l'événement (YYYY-MM-DD HH:MM): "
            )
            start_date += ":00"
            try:
                ...
                EventView.display_error_message(
                    "Format de date incorrect. Utilisez le format YYYY-MM-DD HH:MM."
                )
    ...
    def input_end_date():
        while True:
            end_date = input(
                "Entrez la date de fin de l'événement (YYYY-MM-DD HH:MM): "
            )
            end_date += ":00"
            try:
    ...
    def input_notes():
        while True:
            ...
            if len(notes) < 255:
                ...
                EventView.display_error_message(
                    "Veuillez reduire la taille de votre note"
                )
    ...
    def ask_event_update_field_support_id():
        while True:
            answer = input(
                "Voulez-vous modifier le support id de l'evenement (oui/non)?"
            )
            if answer == "oui":
                return True
            elif answer == "non":
        ...
        while True:
            try:
                ...
                if choice == 1:
                    ...
                    EventView.display_warning_message(

...
class EventController:
            ...
            if not event:
                if contract.client.get_commercial_id() == user_id:
                    if contract.signed:
                        ...
                        session.commit()
                        capture_message(
                            f"Création événement:{new_event.id}", level="info"
                        )
                ...
                self.view.display_warning_message(
                    "Un événement est déjà associé à ce contrat"
                )
        else:
    ...
    def read_all_events(self):
        try:
            ...
            print(
                f"Une erreur s'est produite lors de la récupération des événements : {e}"
            )
        ...
        if event:
            ...
            if user_role == "gestion":
                ...
                if support_id:
                    ...
                    session.commit()
                    capture_message(
                        f"Modification contact support événement:{event.id}",
                        level="info",
                    )
                    self.view.display_info_message(
                        "Contact support modifié avec succès."
                    )
                ...
                if event.get_support_id() == user_id:
                    ...
                    if choice == "date_debut":
                        ...
                        session.commit()
                        capture_message(
                            f"Modification date début événement:{event.id}",
                            level="info",
                        )
                        self.view.display_info_message(
                        ...
                        session.commit()
                        capture_message(
                            f"Modification date fin événement:{event.id}", level="info"
                        )
                        self.view.display_info_message(
                        ...
                        session.commit()
                        capture_message(
                            f"Modification signature événement:{event.id}", level="info"
                        )
                        self.view.display_info_message(
                        ...
                        session.commit()
                        capture_message(
                            f"Modification participants événement:{event.id}",
                            level="info",
                        )
                        self.view.display_info_message(
                        ...
                        session.commit()
                        capture_message(
                            f"Modification note événement:{event.id}", level="info"
                        )
                        ...
                        session.commit()
                        capture_message(
                            f"Modification contrat événement:{event.id}", level="info"
                        )
                        self.view.display_info_message(
                            "ID contrat modifié avec succès."
                        )
    ...
    def filter_events(self, user_role, user_id):
        if user_role == "support":
            ...
            try:
                ...
                self.view.display_error_message(
                    f"Une erreur s'est produite lors de la récupération des événements :"
                    f" {e}"
                )

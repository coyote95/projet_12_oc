...
def test_create_client(init_session):
    ...
    mock_client_view.input_info_client.return_value = (
        "tarault",
        "manon",
        "manon@test.com",
        "0123654789",
...
def test_delete_client_by_id(all_instances):
    ...
    mock_client_view.display_info_message.assert_called_once_with(
...
def test_delete_client_by_id_not_found(all_instances):
    ...
    mock_client_view.input_id_client.assert_called_once()
    mock_client_view.display_warning_message.assert_called_once_with(
        "Client non trouvé."
    )
...
def test_update_name_client(all_instances):
    ...
    mock_client_view.display_info_message.assert_called_once_with(
        "Nom du client modifié avec succès."
    )

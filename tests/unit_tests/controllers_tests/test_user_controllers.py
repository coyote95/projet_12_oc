...
def test_create_user(init_session):
    ...
    mock_user_view.input_infos_user.return_value = (
        "John",
        "j1hn@example.com",
        "commercial",
        "password",
...
def test_delete_user_by_id(all_instances):
    ...
    mock_user_view.display_info_message.assert_called_once_with(
...
def test_delete_user_by_id_not_found(init_session):
    ...
    user_controller.delete_user_by_id()
    mock_user_view.input_id_user.assert_called_once()
    mock_user_view.display_warning_message.assert_called_once_with(
